...
export function CardCreated(props) {
    return (<div className="flex flex-col  items-center  max-[500px]:items-start">
        <div className="front w-6/12 p-4 h-48 mt-20 rounded-xl text-md mb-8 bg-card-front-design border-black text-white max-[500px]:order-last  max-[500px]:h-52 max-[500px]:w-10/12 max-[500px]:order-2">
            ...
            <div className="relative inset-y-12 inset-x-6 flex justify-start text-xl tracking-wide  leading-10 max-[600px]:text-sm max-[600px]:leading max-[600px]:tracking-0">
                {props.cardNum}
            </div>
            <div className="relative inset-y-16 inset-x-6 flex  justify-between ">
                <div className="uppercase">{props.cardName}</div>
                <div className="mr-8">{props.month}/{props.year}</div>
